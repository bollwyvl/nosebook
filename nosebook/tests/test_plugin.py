...
def match_cell(pattern):
    return ["--nosebook-match-cell", pattern]


def scrub(patterns):
    return ["--nosebook-scrub", json.dumps(patterns)]
...
class TestMatchCell(TestNosebook):
    ...
    """
    args = match("Test") + match_cell(
        r"^\s*(class|def) .*[tT]est.*") + OTHER_ARGS
